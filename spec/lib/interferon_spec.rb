...
describe Interferon::Destinations::Datadog do
  ...
  shared_examples_for 'alert_option' do |alert_option, same_value, different_value, alert_dsl|
    ...
    let(:alert_same) do
      mock_alert_json(
        'name2', 'testquery', json_message, 'metric alert', [1], ['tag1'], alert_option => same_value
      )
    end
    let(:alert_diff) do
      mock_alert_json(
        'name2', 'testquery', json_message, 'metric_alert', [1], ['tag1'], alert_option => different_value
      )
    end
  ...
  describe '#same_alerts' do
    ...
    it 'detects no change if alert silenced is true compared to wildcard hash' do
      alert1 = create_test_alert('name1', 'testquery', 'message', ['tag1'], 'silenced' => true)
      alert2 = mock_alert_json(
        'name2', 'testquery', json_message, 'metric alert', [1], ['tag1'], 'silenced' => { '*' => nil }
      )
  ...
  def mock_alert_json(name, datadog_query, message, type = 'metric alert', id = nil, tags = [], options = {})
